...
import VoiceEngineLoading from '../components/VoiceEngineLoading'; // 新增：离线引擎下载进度UI
...
const HomeScreen = ({ navigation }: any) => {
    ...
    useEffect(() => {
        ...
        // 🧠 [修改] 为了展现动态加载效果和节省流量，移除了此处的默认启动，改为用户点击语音按钮时懒加载
    ...
    // --- 启动原声录音监听 (核心改造：加入模型懒加载下载环节) ---
    const startVoiceAssistant = async () => {
        try {
            // [新增]: 如果引擎未初始化，弹出全屏进度条让引擎先去下载并初始化
            if (!voiceInferenceService.isInitialized) {
                ...
                return; // 如果进入了首次初始化分支，则结束本次调用，等待 setTimeout 内回调
            }
