...
export default function MerchantLayout() {
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-[#f8f7f5]">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-gray-500 text-sm">加载商家工作台...</p>
        </div>
      </div>
    );
  }
  ...
  if (!merchant) {
    return (
      <div className="flex h-screen items-center justify-center bg-[#f8f7f5] flex-col gap-4">
        <p className="text-gray-500">无法加载店铺信息。</p>
        <button onClick={() => window.location.reload()} className="text-orange-500 underline">重试</button>
      </div>
    );
  }

  return (
    <div className="flex h-screen w-full bg-[#f8f7f5] dark:bg-[#221710] font-sans text-slate-800 antialiased overflow-hidden">
      
      {/* 侧边栏组件 */}
      <MerchantSidebar user={user} />

      {/* 主内容区域 */}
      <div className="flex flex-col flex-1 h-full min-w-0 bg-[#f8f7f5] dark:bg-background-dark">
        {/* 顶部栏组件 */}
